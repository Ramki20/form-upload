...
public class DisbursementBS_UT extends DLSExternalCommonTestMockBase
{
	...
	{
		super.setUp();
		disbursement = new DisbursementBS();
	}
	...
	public void testCreateAPaymentRequestUnhappy1() throws DLSBusinessServiceException
	{
		...
		catch (DLSBCInvalidDataStopException ex)
		{
			...
			// the expected.
			assertTrue("Expected 1 or 20 error messages", 
					ex.getErrorMessageList().size() == 1 || ex.getErrorMessageList().size() == 20);
		}
	}
	...
	public void testCreateAPaymentRequest() throws DLSBusinessServiceException
	{
		...
		try
		{
			...
			assertTrue("Payment ID should be greater than 0", resp.getPaymentId() > 0);
		}
		catch (DLSBCInvalidDataStopException ex)
		{
			// Exception would occur if there is already a payment with same uri.
			assertEquals("Expected exactly 1 error message", 1, ex.getErrorMessageList().size());
		}
	}
	...
	public void testCreateAPaymentRequestUnhappyNull() throws DLSBusinessServiceException
	{
		...
		catch (DLSBCInvalidDataStopException ex)
		{
			...
			// the expected.
			assertTrue("Expected 1 or 21 error messages", 
					ex.getErrorMessageList().size() == 1 || ex.getErrorMessageList().size() == 21);
		}
	}
	...
	public void testCancelPaymentRequest() throws DLSBCInvalidDataStopException, DLSBusinessServiceException
	{
		...
		try
		{
			...
			assertNotNull("DisbursementResponseBO should not be null", disbursementResponseBO);

			// id of disbursementResponseBO should not be null. Expected.
			assertNotNull("Payment ID should not be null", disbursementResponseBO.getPaymentId());

			// Status should change to cancel(CU). Expected.
			assertTrue("Status should be CU", disbursementResponseBO.getStatus().equalsIgnoreCase("CU"));
		}
		catch (DLSBCInvalidDataStopException ex)
		{
			...
			DLSBusinessServiceException
	{
		...
		{
			assertEquals("Expected exactly 3 error messages", 3, ex.getErrorMessageList().size());
		}
	}
			...
			DLSBusinessServiceException
	{
		...
		{
			assertEquals("Expected exactly 6 error messages", 6, ex.getErrorMessageList().size());
		}
	}
	...
	public void testRetrievePaymentRequestByURI() throws Exception{
		...
		assertNotNull("Unique request identifier should not be null", disbursementResponseBO.getUniqueRequestIdentifier());
		assertTrue("Payment ID should be greater than 0", disbursementResponseBO.getPaymentId() > 0);
	}
			...
			DLSBusinessServiceException
	{
		...
		{
			assertEquals("Expected exactly 5 error messages", 5, ex.getErrorMessageList().size());
		}
	}
	...
	public void testCreateBatchPaymentRequest() throws DLSBusinessServiceException
	{
		...
		try
		{
			...
			assertEquals("Expected map size to be 1", 1, batchDisbursementResponseBOMap.size());
		}
		catch (DLSBCInvalidDataStopException ex)
		{
			assertEquals("Expected exactly 1 error message", 1, ex.getErrorMessageList().size());
