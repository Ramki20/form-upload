...
public abstract class DLSReadonlyJdbcDaoSupport<BO extends BusinessObjectBase, Key extends Serializable>
	...
	// FIXED: Convert to use NamedParameterJdbcTemplate with proper parameter binding
	protected List<BO> retrieve(String retrieveSql, List<Object> ids,
			RowMapper<BO> rowMapper) throws DLSPersistenceFatalException {
		List<BO> domainObjects = new ArrayList<BO>();
		String msg = "";
		try {
			// Convert to named parameters approach
			MapSqlParameterSource paramSource = new MapSqlParameterSource();
			
			// Create parameter names for the list of IDs
			StringBuilder sqlBuilder = new StringBuilder(retrieveSql);
			for (int index = 0; index < ids.size(); index++) {
				String paramName = "id" + index;
				paramSource.addValue(paramName, ids.get(index));
			}
			
			// Note: You may need to modify the SQL to use named parameters like :id0, :id1, etc.
			// instead of positional parameters (?)
			
			NamedParameterJdbcTemplate jdbcTemplate = new DAOUtil().validateNamedParameterJdbcTemplate(this.getNamedParameterJdbcTemplate());
			domainObjects = jdbcTemplate.query(retrieveSql, paramSource, rowMapper);
